...
class TrackView(Gtk.DrawingArea):
			...
			if self.parent.seq.index in mod.extras:
				if self.trk.index in mod.extras[self.parent.seq.index]:
					if "track_show_notes" in mod.extras[self.parent.seq.index][self.trk.index]:
						self.show_timeshift = mod.extras[self.parent.seq.index][self.trk.index]["track_show_timeshift"]
						self.show_pitchwheel = mod.extras[self.parent.seq.index][self.trk.index]["track_show_pitchwheel"]
						self.show_controllers = mod.extras[self.parent.seq.index][self.trk.index]["track_show_controllers"]
						self.show_notes = mod.extras[self.parent.seq.index][self.trk.index]["track_show_notes"]
			else:
	...
	def configure(self):
		...
		if recr_cr:
			...
			self.zero_pattern_surface = self._back_surface.create_similar(cairo.CONTENT_COLOR_ALPHA, round(self.width), round(self.txt_height * 2))
			...
			self.empty_pattern_surface = self._back_surface.create_similar(cairo.CONTENT_COLOR_ALPHA, round(self.width), round(self.txt_height * empl))
