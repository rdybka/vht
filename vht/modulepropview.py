...
class ModulePropView(Gtk.Box):
	def __init__(self, seqview):
		...
		self.highlight_adj.set_value(cfg.highlight)
		...
		self.octave_adj.set_value(cfg.octave)
		...
		self.skip_adj.set_value(cfg.skip)
		...
		self.vel_adj.set_value(cfg.velocity)
	...
	def on_highlight_changed(self, adj):
		cfg.highlight = int(adj.get_value())
		self.seqview.redraw_track(None)

	def on_octave_changed(self, adj):
		cfg.octave = int(adj.get_value())

	def on_skip_changed(self, adj):
		cfg.skip = int(adj.get_value())

	def on_vel_changed(self, adj):
		cfg.velocity = int(adj.get_value())
